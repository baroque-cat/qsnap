# Bucket FULL Backup Strategy

## Purpose

Encapsulates bucket-driven FULL backup creation decisions behind an `IBucketFullStrategy` ABC. Core delegates the decision to the strategy via the factory, eliminating business logic from the orchestrator.
...
The system SHALL provide a `BucketFullStrategy` class in `qsnap/modules/backup/bucket_strategy.py` that implements `IBucketFullStrategy`. It SHALL be a stateless worker — constructor accepts no dependencies, and all logic is self-contained (pure computation on input parameters). It SHALL NOT inherit from Core.
