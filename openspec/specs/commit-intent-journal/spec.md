...
WAL-lite journal of in-flight commit intents persisted in per-VM state. Core writes an
intent record before any blockcommit/commit command and clears it only after the outcome is
finalized, so a crash or client timeout leaves enough evidence to reconcile on the next run.
