...
Monitoring and visibility for deferred blockcommit operations accumulated due to AppArmor/SELinux denial. Provides configurable thresholds with WARNING/CRITICAL log alerts at end of pipeline, a `qsnap list deferred` CLI command, remediation guidance in `qsnap check`, and a `last_warned_at` field for future notification deduplication.
