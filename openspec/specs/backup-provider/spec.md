...
`BitmapBackupProvider.transfer_missing()` SHALL perform post-transfer verification according to `target.verify` via `verify_bitmap_incremental()` for incrementals (backing-filename check + dirty-size regression barrier) and `verify_full_backup()` for the full-pull defensive fallback (no prior checkpoint — standalone qcow2, verified as a FULL). `"off"` skips verification. Every verification failure SHALL produce `BackupResult(success=False, error="verification failed: ...")`.

#### Scenario: Metadata verification passes
- **WHEN** target.verify is "metadata" and the delta passes structural checks (format, virtual-size, backing-filename, dirty-size barrier)
