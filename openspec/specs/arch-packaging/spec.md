...
PKGBUILD for Arch Linux system-package installation — installs qsnap to system Python site-packages so that system-installed `libnbd` bindings are discoverable on `sys.path` without venv configuration.
