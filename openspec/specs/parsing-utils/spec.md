...
The system SHALL provide a `parse_timestamp(name: str, filepath: Path) -> datetime` function in `qsnap/utils/parsing.py`. It SHALL extract the timestamp from the filename using regex-based pattern matching, supporting the unified timestamp format `YYYYMMDDTHHMMSS` (e.g. `20250713T153123`) — seconds resolution, no timezone offset.
