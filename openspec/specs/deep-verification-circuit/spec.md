...
`BlockCommitManager.blockcommit()` SHALL accept an optional `deep_verify: bool = False` keyword argument. When `True`, after a successful blockcommit the manager SHALL run `qemu-img check --output=json` on the base image. If `corruptions > 0`, the `CommitResult` SHALL be `success=False` with the corruption count in the error message.
...
#### Scenario: deep_verify fails after deferred commit
...
`VMConfig` SHALL include `blockcommit_deep_verify: bool` (default `False`) and `snapshot_deep_verify: bool` (default `False`) fields.
