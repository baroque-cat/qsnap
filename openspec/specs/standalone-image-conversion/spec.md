...
Stateless utility functions in `qsnap/utils/convert.py` that flatten a qcow2 backing chain into a standalone image (`qemu-img convert --force-share`), verify the converted output (virtual-size equality + `qemu-img check`), and apply the backup retry policy around the conversion. Shared by `Core.fork()` and `Core.restore()` so both paths use identical conversion, verification, retry, and partial-file cleanup behavior. All external commands go through the injected `IShell`; the helpers touch no state, config, or libvirt objects.
