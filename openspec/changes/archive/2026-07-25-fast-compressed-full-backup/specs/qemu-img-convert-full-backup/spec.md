...
`BitmapBackupProvider` SHALL use `qemu-img convert` as the FULL backup transfer engine, replacing the Python `pread`/`pwrite` loop + write-side `qemu-nbd` with `driver=compress`. The command SHALL be executed via `IShell.run_with_stall_detection()` with the target `.tmp` file as `output_file` and `target.backup_stall_timeout` as `stall_timeout`.

For **running VMs**, the command SHALL read from the NBD source socket started by `virsh backup-begin`:
```
qemu-img convert [-c] -O qcow2 [-o compression_type=<type>] -m 4 -W -p nbd:unix:<socket> <target>.tmp
```

For **stopped VMs**, the command SHALL read directly from the source qcow2 file:
```
qemu-img convert [-c] -O qcow2 [-o compression_type=<type>] -m 4 -W -p <source>.qcow2 <target>.tmp
```
...
The `-m 4` flag (4 parallel coroutines) and `-W` flag (out-of-order writes) SHALL always be included for optimal throughput. The `-p` flag (progress bar) SHALL be included.
...
#### Scenario: Running VM FULL with zstd compression
...
#### Scenario: Stopped VM FULL with compression

- **WHEN** `create_full_backup("myvm", ..., compress=True, compression_type="zstd")` is called and the VM is shut off
- **THEN** `virsh backup-begin` is NOT called
- **THEN** `qemu-img convert -c -O qcow2 -o compression_type=zstd -m 4 -W -p <source>.qcow2 <target>.tmp` is executed
...
`BitmapBackupProvider.create_full_backup()` SHALL call `is_vm_running()` before choosing the transfer path. When the VM is running, the method SHALL use `virsh backup-begin` + `qemu-img convert nbd:unix:<socket>`. When the VM is stopped, the method SHALL use direct `qemu-img convert <source_path> <target>` without `virsh backup-begin`.

The source path for stopped-VM conversion SHALL be resolved via `get_first_disk_path()` — a new helper that returns the file path of the VM's first disk (not the target device name). This is analogous to `get_first_disk_target()` but returns the file path.
...
A new helper function `get_first_disk_path(shell: IShell, vm_name: str) -> str` SHALL be added to `qsnap/utils/nbd.py`. It SHALL parse `virsh domblklist --domain <vm_name> --details` output and return the file path of the first disk (the "Source" column for the first disk with Type "disk"). This is analogous to `get_first_disk_target()` but returns the file path instead of the target device name.

#### Scenario: Returns path for first disk

- **WHEN** `get_first_disk_path(shell, "myvm")` is called
- **AND** `virsh domblklist --domain myvm --details` returns `vda /path/to/disk.qcow2 disk`
