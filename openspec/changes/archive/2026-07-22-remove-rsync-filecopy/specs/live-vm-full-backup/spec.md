...
**Reason**: The method selection existed to choose between the NBD pull-model and `FileCopyBackupProvider`'s stopped-VM direct-convert path. The provider and its direct-convert path are deleted; there is a single NBD path and nothing to select.
**Migration**: Replaced by the "FULL backup requires a running VM" requirement below. Stopped-VM FULL backups are not supported in this change (accepted BREAKING); they return `BackupResult(success=False)`.

### Requirement: Libvirt version check for NBD FULL path
**Reason**: The libvirt < 6.0 fallback to direct `qemu-img convert` died with the file-copy provider. Platform gating is now a hard factory/env-validation concern (libvirt >= 7.2).
**Migration**: See the `module-factory` capability — insufficient libvirt is a `RuntimeError`, not a fallback.
