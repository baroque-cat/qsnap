...
**Reason**: Automatic recovery removed by decision — a broken chain aborts the VM pipeline and requires operator intervention (`qsnap check --deep`). `_split_at_break` and the partial-commit branch are deleted.
**Migration**: None (no state/config migration). Operators must repair broken chains manually before qsnap resumes blockcommits for the affected VM.

### Requirement: Auto-rebase for stuck snapshots onto the disk's base image
**Reason**: `qemu-img rebase -u` auto-recovery removed together with partial blockcommit — too risky to run unattended. `_auto_rebase_stuck` is deleted.
**Migration**: None. Stuck snapshots behind a broken file stay in place until the operator repairs the chain.

### Requirement: Split-at-break walks the disk's backing chain
**Reason**: `_split_at_break` deleted together with the partial-blockcommit feature it served.
**Migration**: None.
