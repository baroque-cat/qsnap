...
- **WHEN** `run_with_stall_detection(cmd, output_file=Path("/tmp/backup.qcow2.tmp"), stall_timeout=1800)` is called
- **AND** the process is running but `output_file` size has not increased for 1800 seconds (30 minutes)
...
- **WHEN** `run_with_stall_detection(cmd, output_file=Path("/tmp/backup.qcow2.tmp"), stall_timeout=1800)` is called
- **AND** the process is running and `output_file` size increases by 1 MB every 60 seconds (slow but progressing)
...
- **THEN** the failure is logged at DEBUG level, not ERROR (same as `run(check=True)`)
...
- **AND** a background process writes 1KB to `/tmp/growing.tmp` every 30 seconds
