...
def test_reconcile_skips_non_qsnap_files_on_target(
    ...
    with (
        patch.object(mock_factory._bitmap_backup_provider, "list", return_value=[non_qsnap]),
        patch.object(
...
def test_reconcile_removes_orphan_snapshot_files(
    make_vm_config, make_target, mock_factory, mock_state, mock_shell, success_result
):
    ...
    mock_shell.expect_first("--backing-chain").returns(
        success_result('[{"format": "qcow2", "filename": "' + str(orphan_backup.path) + '"}]')
    )
    mock_shell.expect("qemu-img info --output=json").returns(success_result("{}"))
    with (
        patch.object(mock_factory._bitmap_backup_provider, "list", return_value=[orphan_backup]),
        patch.object(
            mock_factory._bitmap_backup_provider,
            "delete",
            wraps=mock_factory._bitmap_backup_provider.delete,
...
def test_reconcile_orphan_file_cleanup_non_fatal(
    ...
    core = Core(config=config, factory=mock_factory, state=mock_state, shell=mock_shell)
    with (
        patch.object(core, "_detect_broken_chains", return_value=[]),
        patch.object(
            mock_factory._bitmap_backup_provider,
            "list",
    ...
    assert result["testvm"].orphan_files_removed == 0
    assert any("error checking orphan files" in r.message for r in caplog.records), (
...
def test_reconcile_cleans_dependency_records_on_orphan_deletion(
    ...
    assert orphan_name in deps, (
        f"Dependency {orphan_name} → {full_name} should have been supplemented into state (D2)"
    )
    assert result["testvm"].broken_chains == []
...
def test_reconcile_detects_broken_chain_before_orphan(
    make_vm_config, make_target, mock_factory, mock_state, mock_shell, caplog, failure_result
):
    ...
    assert any("broken chain" in r.message for r in critical_logs), (
...
def test_reconcile_intact_chains_no_broken_chains(
    ...
    mock_shell.expect_first("--backing-chain").returns(
        success_result('[{"format": "qcow2", "filename": "' + str(inc_path) + '"}]')
    )
    with (
        patch.object(
            mock_factory._bitmap_backup_provider,
...
def test_reconcile_dry_run_reports_broken_chains_no_deletion(
    ...
    assert orphan_name in result["testvm"].broken_chains, (
        f"Broken chain for {orphan_name} should be in broken_chains even in dry-run mode"
    )
    critical_logs = [r for r in caplog.records if r.levelno >= logging.CRITICAL]
