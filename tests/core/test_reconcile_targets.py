...
def _no_backing_json(file_path: Path) -> str:
    ...
    return json.dumps({'filename': str(file_path), 'format': 'qcow2', 'virtual-size': 10737418240, 'actual-size': 200704})

@pytest.mark.unit
@pytest.mark.mock
def test_reconcile_phantom_full_removed(make_vm_config, make_target, mock_factory, mock_state, mock_shell, tmp_path):
    ...
    mock_state._full_backups[str(target.path)] = [FullBackupInfo(name=full1_name, path=full1_path, timestamp=datetime.now(), disk='vda'), FullBackupInfo(name=full2_name, path=full2_path, timestamp=datetime.now(), disk='vda')]
    mock_state.record_incremental_dependency(str(target.path), 'testvm.20250725T1200_vda', full1_name)
...
@pytest.mark.unit
@pytest.mark.mock
def test_reconcile_stale_dep_removed(make_vm_config, make_target, mock_factory, mock_state, mock_shell, tmp_path):
    ...
    mock_state._full_backups[str(target.path)] = [FullBackupInfo(name=full_name, path=full_path, timestamp=datetime.now(), disk='vda')]
...
@pytest.mark.unit
@pytest.mark.mock
def test_reconcile_orphan_backup_recorded(make_vm_config, make_target, mock_factory, mock_state, mock_shell, tmp_path, success_result):
    ...
    full_snap = SnapshotInfo(name=full_stem, path=full_path, timestamp=datetime.now(), allocation=0, disk='vda')
    inc1_snap = SnapshotInfo(name=inc1_name, path=inc1_path, timestamp=datetime.now(), allocation=0, disk='vda')
    inc2_snap = SnapshotInfo(name=inc2_name, path=inc2_path, timestamp=datetime.now(), allocation=0, disk='vda')
    mock_shell.expect_first('--backing-chain').returns(success_result('[{"format": "qcow2", "filename": "' + str(inc2_path) + '"}]'))
    mock_shell.expect('qemu-img info --output=json').returns(success_result(_anchor_json(inc2_path, full_path)))
    with patch.object(mock_factory._bitmap_backup_provider, 'list', return_value=[full_snap, inc1_snap, inc2_snap]):
        result = core.reconcile()
    assert result['testvm'].state_supplemented >= 1
    deps = mock_state.get_incremental_dependencies(str(target.path), full_name)
    assert inc2_name in deps, f'inc2 ({inc2_name}) should be recorded as dependency of FULL after reconcile'
    assert result['testvm'].orphan_files_removed == 0
    assert result['testvm'].broken_chains == []

@pytest.mark.unit
@pytest.mark.mock
def test_reconcile_orphan_broken_chain_critical_not_deleted(make_vm_config, make_target, mock_factory, mock_state, mock_shell, tmp_path, caplog, failure_result):
    """State: FULL + inc1; Disk: FULL + inc1 + inc2 (inc2 not in state);
    ...
    with patch.object(mock_factory._bitmap_backup_provider, 'list', return_value=[full_snap, inc1_snap, inc2_snap]), patch.object(mock_factory._bitmap_backup_provider, 'delete', wraps=mock_factory._bitmap_backup_provider.delete) as delete_spy, caplog.at_level(logging.CRITICAL):
        result = core.reconcile()
...
@pytest.mark.unit
@pytest.mark.mock
def test_reconcile_orphan_checkpoint_deleted(make_vm_config, make_target, mock_factory, mock_state, mock_shell, caplog, success_result):
    ...
    with patch.object(mock_factory._bitmap_backup_provider, 'list_checkpoints', return_value=[orphan_checkpoint]) as list_spy, caplog.at_level(logging.INFO):
        result = core.reconcile()
...
@pytest.mark.unit
@pytest.mark.mock
def test_reconcile_stale_baseline_cleared(make_vm_config, make_target, mock_factory, mock_state, mock_shell):
    ...
    assert mock_state.get_last_backup_allocation(str(target.path), 'vda') is None, 'last_backup_allocation should be cleared when no FULLs remain'

@pytest.mark.unit
@pytest.mark.mock
def test_reconcile_orphan_no_anchor_deleted(make_vm_config, make_target, mock_factory, mock_state, mock_shell, tmp_path, success_result):
    """State: FULL + inc1; Disk: FULL + inc1 + inc2 (inc2 not in state);
    ...
    inc2_snap = SnapshotInfo(name=inc2_name, path=inc2_path, timestamp=datetime.now(), allocation=0, disk='vda')
    mock_shell.expect_first('--backing-chain').returns(success_result('[{"format": "qcow2", "filename": "' + str(inc2_path) + '"}]'))
    mock_shell.expect('qemu-img info --output=json').returns(success_result(_no_backing_json(inc2_path)))
    with patch.object(mock_factory._bitmap_backup_provider, 'list', return_value=[full_snap, inc1_snap, inc2_snap]), patch.object(mock_factory._bitmap_backup_provider, 'delete', wraps=mock_factory._bitmap_backup_provider.delete) as delete_spy:
        result = core.reconcile()
...
@pytest.mark.unit
@pytest.mark.mock
def test_reconcile_after_retention_no_action(make_vm_config, make_target, mock_factory, mock_state, mock_shell, tmp_path):
    ...
    full_snap = SnapshotInfo(name=full_stem, path=full_path, timestamp=datetime.now(), allocation=0, disk='vda')
    inc_snap = SnapshotInfo(name=inc_name, path=inc_path, timestamp=datetime.now(), allocation=0, disk='vda')
    with patch.object(mock_factory._bitmap_backup_provider, 'list', return_value=[full_snap, inc_snap]), patch.object(mock_factory._bitmap_backup_provider, 'delete', wraps=mock_factory._bitmap_backup_provider.delete) as delete_spy:
        result = core.reconcile()
...
@pytest.mark.unit
@pytest.mark.mock
def test_reconcile_dry_run_targets(make_vm_config, make_target, mock_factory, mock_state, mock_shell, tmp_path, caplog, success_result):
    ...
    full2_snap = SnapshotInfo(name=full2_stem, path=full2_path, timestamp=datetime.now(), allocation=0, disk='vda')
    orphan_snap = SnapshotInfo(name=orphan_name, path=orphan_path, timestamp=datetime.now(), allocation=0, disk='vda')
    mock_shell.expect_first('--backing-chain').returns(success_result('[{"format": "qcow2", "filename": "' + str(orphan_path) + '"}]'))
    mock_shell.expect('qemu-img info --output=json').returns(success_result(_anchor_json(orphan_path, full2_path)))
    ...
    with patch.object(mock_factory._bitmap_backup_provider, 'list', return_value=[full2_snap, orphan_snap]), patch.object(mock_factory._bitmap_backup_provider, 'delete', wraps=mock_factory._bitmap_backup_provider.delete) as delete_spy, caplog.at_level(logging.INFO):
        result = core.reconcile()
