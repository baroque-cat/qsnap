...
qcow2 via ``qemu-img convert --force-share -O qcow2``.
...
@pytest.mark.unit
def test_fork_creates_standalone_qcow2_from_snapshot(tmp_path, make_vm_config, mock_factory, mock_state, mock_shell):
...
@pytest.mark.unit
def test_fork_from_backup_resolves_via_backup_provider(tmp_path, make_vm_config, make_target, mock_factory, mock_state, mock_shell):
...
@pytest.mark.unit
def test_fork_flattens_incremental_chain(tmp_path, make_vm_config, mock_factory, mock_state, mock_shell):
    ...
    config = MockConfigFacade(vms=[vm])
    core = Core(config=config, factory=mock_factory, state=mock_state, shell=mock_shell)
    snap = SnapshotInfo(name='inc_backup1', path=Path('/backups/target/inc_backup1.qcow2'), timestamp=datetime(2025, 7, 14, 10, 0), allocation=524288, disk='vda')
    ...
    assert result.success is True
    convert_calls = [' '.join(c.args[0]) for c in spy.call_args_list if 'convert' in ' '.join(c.args[0])]
    assert len(convert_calls) >= 1, 'qemu-img convert should flatten chain'

@pytest.mark.unit
def test_fork_logs_chain_size_before_convert(tmp_path, make_vm_config, mock_factory, mock_state, mock_shell, caplog):
    ...
    with patch.object(mock_shell, 'run', wraps=mock_shell.run) as spy:
        result = core.fork('snap1', output_path)
    assert result.success is True
    assert 'chain size' in caplog.text.lower() or 'Converting snapshot' in caplog.text, f'Expected conversion log message, got: {caplog.text}'
    ...
    if backing_idx is not None and convert_idx is not None:
        assert backing_idx < convert_idx, 'qemu-img info --backing-chain must run before qemu-img convert'

@pytest.mark.unit
def test_fork_chain_size_estimation_uses_force_share(tmp_path, make_vm_config, mock_factory, mock_state, mock_shell):
    ...
    with patch.object(mock_shell, 'run', wraps=mock_shell.run) as spy:
        result = core.fork('snap1', output_path)
    assert result.success is True
    backing_calls = [' '.join(c.args[0]) for c in spy.call_args_list if 'backing-chain' in ' '.join(c.args[0])]
    assert len(backing_calls) >= 1, 'qemu-img info --backing-chain should be called'
    backing_cmd = backing_calls[0]
    assert '--force-share' in backing_cmd, 'qemu-img info --backing-chain must include --force-share'

@pytest.mark.unit
def test_fork_snapshot_not_found_returns_failure(make_vm_config, mock_factory, mock_state, mock_shell):
...
@pytest.mark.unit
def test_fork_returns_restore_result_on_success(tmp_path, make_vm_config, mock_factory, mock_state, mock_shell):
    """fork() returns RestoreResult with success=True and correct paths."""
    vm = make_vm_config(name='sourcevm')
    config = MockConfigFacade(vms=[vm])
    core = Core(config=config, factory=mock_factory, state=mock_state, shell=mock_shell)
    mock_state.record_snapshot('sourcevm', _make_snapshot('snap1'))
...
@pytest.mark.unit
def test_fork_passes_vm_filter_correctly(tmp_path, make_vm_config, mock_factory, mock_state, mock_shell):
    ...
    snap2 = SnapshotInfo(name='snap1', path=Path('/snapshots/vm2_snap1.qcow2'), timestamp=datetime(2025, 7, 13, 10, 0), allocation=99999, disk='vda')
...
@pytest.mark.unit
def test_fork_convert_failure_returns_error(tmp_path, make_vm_config, mock_factory, mock_state, mock_shell):
...
@pytest.mark.unit
def test_fork_warns_active_layer_inconsistency(tmp_path, make_vm_config, mock_factory, mock_state, mock_shell):
    ...
    config = MockConfigFacade(vms=[vm])
    core = Core(config=config, factory=mock_factory, state=mock_state, shell=mock_shell)
    snap = SnapshotInfo(name='active_snap1', path=Path('/var/lib/libvirt/snapshots/testvm/snap4.qcow2'), timestamp=datetime(2025, 7, 13, 14, 0), allocation=1048576, disk='vda')
    mock_state.record_snapshot('sourcevm', snap)
    mock_shell.expect('qemu-img convert').returns(ShellResult(success=True, stdout='', stderr='', returncode=0, error=None))
    ...
    convert_calls = [c for c in mock_shell.call_history if 'qemu-img convert' in c]
    assert len(convert_calls) >= 1
    assert '--force-share' in convert_calls[0], '--force-share must be used when reading from active layer'
    backing_calls = [c for c in mock_shell.call_history if 'backing-chain' in c]
    assert len(backing_calls) >= 1
    assert '--force-share' in backing_calls[0], '--force-share must be used on backing-chain info for active layer'