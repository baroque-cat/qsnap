...
# ── Fork success path: standalone qcow2 from snapshot ───────────────────
...
def test_fork_creates_standalone_qcow2_from_snapshot(
    ...
    # Mock qemu-img convert to succeed
    ...
    # Verify qemu-img convert was called with correct args
...
# ── Fork flattens incremental chain ─────────────────────────────────────
...
def test_fork_flattens_incremental_chain(
    ...
    # Snapshot with a path that looks like an incremental
    snap = SnapshotInfo(
        name="inc_backup1",
    ...
    # Verify convert was called (flattens chain)
    convert_calls = [
        " ".join(c.args[0])
...
def test_fork_logs_chain_size_before_convert(
    ...
    # Verify log message about chain size / conversion appears
    ...
    # Verify ordering: backing-chain info call comes before convert call
...
# ── Fork uses --force-share for chain size estimation ───────────────────
...
# ── Fork snapshot not found → failure ───────────────────────────────────
...
# ── Fork returns RestoreResult on success ───────────────────────────────
...
# ── Fork passes vm_filter correctly ─────────────────────────────────────
...
def test_fork_passes_vm_filter_correctly(
    ...
    # Filter to vm2 only — should resolve vm2's snapshot
    result = core.fork("snap1", output_path, vm_filter="vm2")

    assert result.success is True
    assert result.snapshot_name == "snap1"

    # Verify the convert command used vm2's snapshot path
    convert_calls = [c for c in mock_shell.call_history if "qemu-img convert" in c]
    assert len(convert_calls) >= 1
    assert "vm2_snap1" in convert_calls[0], (
        "convert should use vm2's snapshot, not vm1's"
    )


# ── Fork conversion failure returns error ───────────────────────────────
...
# ── Fork active layer inconsistency (risk) ────────────────────────────────
...
def test_fork_warns_active_layer_inconsistency(
    ...
    # Snapshot whose path mimics the active layer (matches domblklist default)
    snap = SnapshotInfo(
        name="active_snap1",
    ...
    # Verify --force-share is present in the convert command
    convert_calls = [
    ...
    # Verify --backing-chain info call also uses --force-share
    backing_calls = [
        c for c in mock_shell.call_history if "backing-chain" in c
    ]
