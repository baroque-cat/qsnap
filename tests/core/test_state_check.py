...
def test_check_state_all_snapshots_exist_clean(
    tmp_path: Path, make_vm_config, make_target, mock_factory, mock_state, mock_shell
):
...
def test_check_state_result_includes_orphans():
    ...
    result = StateCheckResult(
        vm_name="testvm", status="orphan_checkpoints", orphan_checkpoints=orphans
    )
...
def test_detect_orphan_checkpoints_no_auto_cleanup_by_default(
    ...
    checkpoint_delete_calls = [
        c for c in run_spy.call_args_list if "checkpoint-delete" in " ".join(c[0][0])
    ]
    assert len(checkpoint_delete_calls) == 0, (
        f"No virsh checkpoint-delete should be called without auto_cleanup, got {len(checkpoint_delete_calls)} calls"
    )


def test_detect_orphan_checkpoints_auto_cleanup_deletes(
    tmp_path: Path, make_vm_config, make_target, mock_factory, mock_state, mock_shell, caplog
):
    ...
    assert len(deleted_logs) == 2, (
        f"Should have 2 INFO logs about deleted orphan checkpoint, got {len(deleted_logs)}"
    )
...
def test_detect_orphan_checkpoints_auto_cleanup_non_fatal(
    ...
    assert len(failed_delete_logs) == 2, (
        f"Should have 2 WARNING logs about failed deletion, got {len(failed_delete_logs)}"
    )
...
def test_check_state_full_backups_skipped_in_chain_validation(
    ...
    assert scan_spy.call_count == 1, (
        f"Expected exactly 1 scan_backing_chain call, got {scan_spy.call_count}"
    )
