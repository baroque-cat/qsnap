...
def test_check_healthy_backing_chain_reports_ok(
    ...
    dumpxml = (
        f'<domain type="kvm"><devices><disk><source file="{snap_path}"/></disk></devices></domain>'
    )
...
def test_check_broken_chain_reports_broken_status(
    ...
    mock_shell.expect_first("--backing-chain").returns(
        ShellResult(
            success=False, stdout="", stderr="error", returncode=1, error="backing file not found"
        )
    )
    result = core.check()
    assert "testvm" in result
...
def test_print_schedule_shows_snapshot_and_backup_retention(
    ...
    vm = make_vm_config(
        name="testvm", snapshot_chain_length=24, targets=[make_target(target_chain_length=24)]
    )
    config = MockConfigFacade(vms=[vm])
    core = Core(config=config, factory=mock_factory, state=mock_state, shell=mock_shell)
    snap = SnapshotInfo(
...
def test_check_deep_clean_image_reports_ok(make_vm_config, mock_factory, mock_state, mock_shell):
    ...
    mock_shell.expect("qemu-img info.*--backing-chain").returns(
        ShellResult(
            success=True,
            stdout=json.dumps([{"filename": "/tmp/snap1.qcow2", "format": "qcow2"}]),
            stderr="",
            returncode=0,
            error=None,
        )
...
def test_list_deferred_returns_per_vm_summaries(
    make_vm_config, mock_factory, mock_state, mock_shell, frozen_clock
):
...
def test_list_backups_tree_true_returns_nested_dict(
    ...
    with (
        patch.object(mock_factory._backup_provider, "list", return_value=[full1, inc1]),
        patch.object(
            core, "_resolve_chain_full_anchor", return_value="testvm.FULL.20250701T120000_abc123"
        ),
    ):
...
def test_list_backups_tree_groups_by_chain(
    ...
    with (
        patch.object(mock_factory._backup_provider, "list", return_value=all_backups),
        patch.object(core, "_resolve_chain_full_anchor", side_effect=mock_resolve),
    ):
        result = core.list_backups(tree=True)
    assert "testvm" in result
    target_chains = result["testvm"]
    ...
    assert chain1_names == sorted(
        ["testvm.FULL.20250701T120000_abc123", "testvm.20250702T120000_def456"]
    )
    chain2_names = sorted(b.name for b in chains["testvm.FULL.20250703T120000_ghi789"])
    assert chain2_names == sorted(
        ["testvm.FULL.20250703T120000_ghi789", "testvm.20250704T120000_jkl012"]
    )
...
def test_list_backups_tree_orphans_under_orphan_key(
    ...
    with (
        patch.object(mock_factory._backup_provider, "list", return_value=all_backups),
        patch.object(core, "_resolve_chain_full_anchor", return_value=None),
    ):
        result = core.list_backups(tree=True)
    assert "testvm" in result
    target_chains = result["testvm"]
...
@pytest.mark.unit
def test_list_latest_multi_disk_both_have_snapshots(
    make_vm_config, mock_factory, mock_state, mock_shell
):
    ...
    for i in range(2):
        snap = SnapshotInfo(
            name=f"vda_snap{i}",
            path=Path(f"/tmp/vda_snap{i}.qcow2"),
            timestamp=base + timedelta(hours=i),
            allocation=1000 * (i + 1),
            disk="vda",
        )
    ...
    for i in range(3):
        snap = SnapshotInfo(
            name=f"vdb_snap{i}",
            path=Path(f"/tmp/vdb_snap{i}.qcow2"),
...
@pytest.mark.unit
def test_list_backups_flat_two_targets(
    make_vm_config, make_target, mock_factory, mock_state, mock_shell
):
    ...
    t1_backups = [
        SnapshotInfo(
            name="t1_b1",
            path=Path("/mnt/backup/testvm/target1/t1_b1.qcow2"),
            timestamp=base,
            allocation=1000,
            disk="vda",
        ),
    ...
    t2_backups = [
        SnapshotInfo(
            name="t2_b1",
            path=Path("/mnt/backup/testvm/target2/t2_b1.qcow2"),
            timestamp=base + timedelta(hours=1),
            allocation=2000,
            disk="vda",
        ),
