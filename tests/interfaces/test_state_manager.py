...
# ── collapse_in_progress phase contract (hysteresis-snapshot-retention) ────
# state-management spec: ``get_collapse_in_progress`` /
# ``set_collapse_in_progress`` / ``clear_collapse_in_progress`` are abstract
# on IStateManager.  Behavior round-trips are parametrized over BOTH concrete
# implementations (JsonStateManager + InMemoryStateManager) so mock and
# production parity is enforced by the contract suite (TESTING.md §3).


def test_istate_manager_collapse_in_progress_methods_abstract():
    """The three collapse-phase methods are abstract on IStateManager.

    A subclass implementing every abstract method EXCEPT
    ``get_collapse_in_progress`` / ``set_collapse_in_progress`` /
    ``clear_collapse_in_progress`` MUST fail to instantiate with TypeError.
    """
    abstract_methods = IStateManager.__abstractmethods__
    assert "get_collapse_in_progress" in abstract_methods
    assert "set_collapse_in_progress" in abstract_methods
    assert "clear_collapse_in_progress" in abstract_methods

    class _MissingCollapseMethods(IStateManager):
        def get_last_allocation(self, vm_name, disk): ...
        def set_last_allocation(self, vm_name, disk, alloc): ...
        def record_snapshot(self, vm_name, info): ...
        def remove_snapshot(self, vm_name, snapshot_name): ...
        ...
        def get_last_commit_ts(self, vm_name, disk): ...
        def set_last_commit_ts(self, vm_name, disk, timestamp): ...
        def set_commit_in_progress(self, vm_name, disk, snapshots, base, started_ts): ...
        def get_commit_in_progress(self, vm_name): ...
        def clear_commit_in_progress(self, vm_name, disk): ...

    with pytest.raises(TypeError):
        _MissingCollapseMethods()


@pytest.mark.parametrize("mgr_cls", [JsonStateManager, InMemoryStateManager])
def test_contract_collapse_in_progress_methods_abstract(mgr_cls, tmp_path):
    """Collapse-phase set/read/clear/reset round-trips in both implementations.

    Mirrors the state-management scenarios (tests/state/test_manager.py
    parity for the mock): missing key reads empty, set is idempotent,
    clear removes exactly one disk, ``reset_vm_state`` clears the list,
    and ``reset_vm_disk_state`` removes one disk.
    """
    mgr = _make_state_manager(mgr_cls, tmp_path)
    ...
    # Set is idempotent; multiple disks accumulate.
    ...
    # Per-disk reset removes one disk; full reset clears the list.
    mgr.set_collapse_in_progress("testvm", "vdc")
    mgr.reset_vm_disk_state("testvm", "vdb")
    assert mgr.get_collapse_in_progress("testvm") == ["vdc"]
