...
qemu-img convert → RestoreResult.  Uses MockShell (no real virsh/qemu-img
...
def _setup_fork_shell(mock_shell, chain_size_json=None):
    ...
    Registers responses for:
      - qemu-img info --backing-chain --output=json (chain size)
...
def test_fork_full_pipeline_creates_standalone_qcow2(
    ...
    """End-to-end: fork resolves snapshot, estimates chain size, converts to standalone qcow2."""
...
def test_fork_pipeline_logs_chain_size(
    ...
    """fork() runs qemu-img info --backing-chain and logs chain size before converting."""
    vm = make_vm_config(name="testvm")
    config = MockConfigFacade(vms=[vm])
    core = Core(config=config, factory=mock_factory, state=mock_state, shell=mock_shell)
...
def test_fork_from_incremental_flattens_chain(
    ...
    """fork() from an incremental backup converts to standalone qcow2 (no backing file).

    Scenario 54: Create FULL + 2 incrementals, fork the incremental,
    verify standalone result (no backing file, --force-share -O qcow2).
    """
    ...
    with patch.object(
        mock_factory._backup_provider, "list", return_value=[inc2_info]
    ):
    ...
    assert "-O" in convert_cmd and "qcow2" in convert_cmd, (
        "convert must output qcow2 format"
    )
    assert str(inc2_info.path) in convert_cmd, (
        "convert must use the incremental backup as source"
    )
