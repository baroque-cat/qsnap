...
def test_source_disk_onchange_gate_skips_when_unchanged(test_vm, caplog, change_detection_mode):
    ...
    # on-disk file, so startup phantom-FULL cleanup cannot drop it and
    # reopen the gate before the second run.
    fulls_after_first = state.get_full_backups(str(target_dir))
    assert len(fulls_after_first) >= 1, "Expected a FULL backup recorded after the first run"
    assert fulls_after_first[0].path.exists(), (
        f"Recorded FULL path must exist after run 1: {fulls_after_first[0].path}"
    )
