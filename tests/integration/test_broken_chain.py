...
# ──────────────────────────────────────────────────────────────────────
# Test 4: Reconcile detects and cleans broken chains (B6)
# ──────────────────────────────────────────────────────────────────────


@pytest.mark.integration
@pytest.mark.timeout(3600)
def test_reconcile_detects_and_cleans_broken_chains(test_vm, caplog):
    """Verify ``Core.reconcile()`` detects and cleans broken-chain orphan files.

    B6 — Before classifying a file as an orphan, ``reconcile()`` checks each
    non-FULL backup's backing-chain integrity via ``qemu-img info
    --backing-chain``.  Broken-chain files are detected and reported in
    ``ReconcileResult.broken_chains``, then deleted as untracked orphans.

    1. Create FULL backup via ``create_full_backup()``, record in state.
    2. On the target, create an untracked .qcow2 file with a broken backing
       chain (qemu-img rebase to nonexistent).
    ...
    provider = BitmapBackupProvider(shell)
    source_snap = SnapshotInfo(
        name=f"{vm_name}.rcl-bc-full-src",
        path=base_image,
        timestamp=datetime.now(),
        allocation=0,
    )
    ...
    # Break the backing chain using unsafe rebase (-u).
    nonexistent = target_dir / "MISSING_FOR_RECONCILE.qcow2"
    shell.run(
        ["qemu-img", "rebase", "-u", "-f", "qcow2", "-F", "qcow2",
         "-b", str(nonexistent), str(orphan_path)],
        timeout=30, check=True,
    )
    assert not _validate_backing_chain(shell, orphan_path), "Orphan chain should be broken"
    ...
    assert vm_name in reconcile_results, f"reconcile should return result for {vm_name}"
    rec_result = reconcile_results[vm_name]

    # ── Step 4: Verify broken chain detection ──────────────────────
    # The orphan should be detected as having a broken chain.
    if len(rec_result.broken_chains) > 0:
        assert any(orphan_name in bc for bc in rec_result.broken_chains), (
            f"broken_chains should mention {orphan_name!r}, "
            f"got: {rec_result.broken_chains}"
        )

    # Check the log for the broken chain warning.
    broken_logs = [
        r.message for r in caplog.records
        if "broken backing chain" in r.message.lower()
    ]
    assert len(broken_logs) > 0, (
        f"Expected 'broken backing chain' log message. "
        f"Logs: {[r.message for r in caplog.records]}"
    )

    # ── Step 5: Verify orphan file was deleted ──────────────────────
    assert not orphan_path.exists(), (
        f"Broken-chain orphan file {orphan_path} should have been deleted by reconcile"
    )
