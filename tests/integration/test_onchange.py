...
# ──────────────────────────────────────────────────────────────────────
# Test 5: Retention runs even when gate skips transfer
# ──────────────────────────────────────────────────────────────────────


@pytest.mark.integration
@pytest.mark.timeout(3600)
def test_retention_runs_on_skip(test_vm, caplog):
    ...
    5. Verify expired backup files are removed from target.
    """
    shell: SubprocessShell = test_vm["shell"]
    vm_name: str = test_vm["vm_name"]
    base_image: Path = test_vm["base_image"]
    ...
    target_normal = TargetConfig(
        path=target_dir, backup_create="always", compress=False, verify="off"
    )
    vm_config_normal = VMConfig(
        name=vm_name, base_image=base_image, snapshot_dir=snapshot_dir, targets=[target_normal]
    )
    config_normal = MockConfigFacade(
        vms=[vm_config_normal], config_path=tmpdir / "onchange_retention_phase1.toml"
    )
    factory = DefaultFactory(shell, state)
    core_normal = Core(config=config_normal, factory=factory, state=state, shell=shell)

    caplog.clear()
    with caplog.at_level(logging.INFO):
        result_phase1 = core_normal.backup(vm_name)
    if result_phase1.results[0].backup_failed:
    ...
    caplog.clear()
    with caplog.at_level(logging.INFO):
        core_aggressive.backup(vm_name)
    ...
    # Verify retention deleted the backup files.
    phase2_count = _count_qcow2_files(target_dir)
    assert phase2_count < phase1_count, (
        f"Phase 2: retention must delete expired backups "
        f"({phase1_count} → {phase2_count}). "
        f"Remaining: {list(target_dir.glob('*.qcow2'))}"
    )
