...
def test_mock_factory_always_returns_bitmap_provider_for_different_target(make_vm_config, make_target):
    """create_backup_provider() always returns MockBitmapBackupProvider
...
def test_mock_backup_provider_has_create_full_backup(make_vm_config, make_target):
    ...
    source_snapshot = SnapshotInfo(name='test-snap', path=Path('/tmp/test-snap.qcow2'), timestamp=datetime.now(), allocation=65536, disk='vda')
...
def test_mock_backup_provider_create_full_backup_accepts_new_params(make_vm_config, make_target):
    ...
    assert isinstance(provider, IBackupProvider)
    source_snapshot = SnapshotInfo(name='test-snap', path=Path('/tmp/test-snap.qcow2'), timestamp=datetime.now(), allocation=65536, disk='vda')
    result = provider.create_full_backup('testvm', source_snapshot, make_target(), compression_type='zstd', stall_timeout=1800)
    assert isinstance(result, BackupResult)
...
def test_mock_backup_provider_transfer_missing_accepts_new_params(make_vm_config, make_target):
    ...
    provider = MockBitmapBackupProvider()
    assert isinstance(provider, IBackupProvider)
    snapshots = [SnapshotInfo(name='inc-1', path=Path('/tmp/inc-1.qcow2'), timestamp=datetime.now(), allocation=65536, disk='vda')]
    results = provider.transfer_missing(make_vm_config(), make_target(), snapshots, compression_type='zstd', stall_timeout=600)
    assert isinstance(results, list)
    assert len(results) == 1
    assert isinstance(results[0], BackupResult)
    assert results[0].success is True

def test_mock_bitmap_backup_provider_create_full_backup_accepts_new_params(make_vm_config, make_target):
    """MockBitmapBackupProvider.create_full_backup() accepts the new
    ``compression_type`` and ``stall_timeout`` keyword arguments
    without error and still passes isinstance(..., IBackupProvider)."""
    provider = MockBitmapBackupProvider()
    assert isinstance(provider, IBackupProvider)
    source_snapshot = SnapshotInfo(name='test-snap', path=Path('/tmp/test-snap.qcow2'), timestamp=datetime.now(), allocation=65536, disk='vda')
    result = provider.create_full_backup('testvm', source_snapshot, make_target(), compression_type='zlib', stall_timeout=0)
    assert isinstance(result, BackupResult)
    assert result.success is True

def test_mock_bitmap_backup_provider_transfer_missing_accepts_new_params(make_vm_config, make_target):
    """MockBitmapBackupProvider.transfer_missing() accepts the new
    ``compression_type`` and ``stall_timeout`` keyword arguments
    without error."""
    provider = MockBitmapBackupProvider()
    assert isinstance(provider, IBackupProvider)
    snapshots = [SnapshotInfo(name='inc-1', path=Path('/tmp/inc-1.qcow2'), timestamp=datetime.now(), allocation=65536, disk='vda')]
    results = provider.transfer_missing(make_vm_config(), make_target(), snapshots, compression_type='zlib', stall_timeout=0)
    assert isinstance(results, list)
    assert len(results) == 1
    assert isinstance(results[0], BackupResult)
    assert results[0].success is True
